...
// (switch_settings_game_folders()) for *.bin files and *.zip archives (each
// holding one .bin - see switch_zip.h), resolving cover art (a
// same-basename *.png next to the game file - see switch/README.md's cover
...
struct LibraryGame {
  std::string full_path;     // sdmc: path to the .bin or .zip, used to launch it
  std::string display_name;  // filename without the .bin/.zip extension
